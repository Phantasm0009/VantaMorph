...
impl VantaMorphApp {
    ...
    pub fn change_sim(
        ...
        // Extract and store the preset's target image if it exists
        if let Some(target_data) = &source.inner.target_img {
            if let Some(img) = image::ImageBuffer::<image::Rgb<u8>, Vec<u8>>::from_raw(
                source.inner.width,
                source.inner.height,
                target_data.clone(),
            ) {
                self.gui.current_preset_target = Some(img);
            }
        } else {
            self.gui.current_preset_target = None;
        }
    ...
    pub fn new(cc: &CreationContext<'_>) -> Self {
        ...
        let presets: Vec<Preset> = get_presets(); // Always load fresh presets from code
        ...
        let has_morphed_once = if let Some(storage) = cc.storage {
            eframe::get_value::<bool>(storage, "has_morphed_once").unwrap_or(false)
        } else {
            false
        };
        ...
        Self {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            stroke_count: 0,
            gui: gui::GuiState::default(presets, random_preset, has_morphed_once),
            frame_count: 0,
            #[cfg(not(target_arch = "wasm32"))]
    ...
    fn ensure_worker(&mut self, _ctx: &egui::Context) {
        ...
        let worker = {
            ...
            .unwrap_or_else(|| {
                // Fallback: parse from Error stack trace to find vantamorph-{hash}.js
                ...
                if let Ok(stack_val) = js_sys::Reflect::get(&error, &JsValue::from_str("stack")) {
                    if let Some(stack) = stack_val.as_string() {
                        // Look for vantamorph-{hash}.js in the stack
                        if let Some(start) = stack.find("vantamorph-") {
            ...
            vec![
                $({
                    ...
                    let target_img = {
                        ...
                        let target_buf = image::load_from_memory(target_bytes)
                            .unwrap()
                            .to_rgb8();
